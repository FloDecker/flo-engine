...
public:
	...
	static std::vector<glm::vec3> get_edges_of_bounding_box(StructBoundingBox* bb,
	                                                        const glm::mat4x4& transformation_matrix);

	static void get_bounding_box_containing_points(StructBoundingBox* bb, const std::vector<glm::vec3>* points);

	static void transform_local_bb_to_world_space_axis_aligned(StructBoundingBox* world_space_result,
	                                                           StructBoundingBox* local_bb,
	                                                           const glm::mat4x4& transformation_matrix);

	//takes two bounding boxes and returns the smallest that contains both
	static void get_combined_bounding_box(StructBoundingBox* resulting_box, const StructBoundingBox* box_a,
	...
	static bool is_in_bounding_box(const StructBoundingBox* bounding_box, const glm::vec3& p,
	                               float uniform_scale_addition = 0.0f);

	//checks if a polygon intersects with a bounding box
	static bool intersects_polygon(const StructBoundingBox* bounding_box, const glm::vec3& v_0, const glm::vec3& v_1,
	                               const glm::vec3& v_2);
	...
	static struct_intersection are_intersecting(const StructBoundingBox* bounding_box_a,
	                                            const StructBoundingBox* bounding_box_b, const glm::mat4& transform_a,
