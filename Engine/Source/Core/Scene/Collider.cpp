...
//                                   bool ignore_back_face, ray_cast_result* ray_cast_hit)
// {
// 	auto global_inverse = inverse(this->transformGlobal);
// 	glm::vec4 ray_cast_origin_vec4_local = global_inverse * glm::vec4(ray_origin_ws, 1);
// 	auto ray_cast_origin_vec3_local = glm::vec3(ray_cast_origin_vec4_local);
//
// 	glm::vec4 ray_cast_direction_vec4_local = global_inverse * glm::vec4(ray_direction_ws, 0);
// 	auto ray_cast_direction_vec3_local = glm::vec3(ray_cast_direction_vec4_local);
// 	check_collision_ls(ray_cast_origin_vec3_local, ray_cast_direction_vec3_local, ray_length, ignore_back_face,
// 	                   ray_cast_hit);
// }
//
...
// 				ray_cast_hit->hit_normal_world_space = this->getGlobalTransform() * glm::vec4(
// 					ray_cast_hit->hit_normal_local, 0);
// 				ray_cast_hit->object_3d = get_parent();
// 				ray_cast_hit->vertex_indices[0] = vertex_array->indices->at(i);
// 				ray_cast_hit->vertex_indices[1] = vertex_array->indices->at(i+1);
// 				ray_cast_hit->vertex_indices[2] = vertex_array->indices->at(i+2);
// 			}
// 		}
// 	}
// }
...
// 		float d = -dot(face_normal, v0.position);
// 		float t =
// 			-(dot(face_normal, proximity_center_local) + d);
//
...
// 		if (length(vec_to_center_global) > radius) continue;
//
// 		auto c = glm::vec3();
//
// 		// Edge 0
...
//
// 	float d = -dot(face_normal, v0.position);
// 	float t = -(dot(face_normal, proximity_center_local) + d);
//
// 	glm::vec3 hit_point = proximity_center_local + t * face_normal;
//
// 	auto vec_to_center_global = this->transformGlobal * glm::vec4(hit_point - proximity_center_local, 0.0);
// 	if (length(vec_to_center_global) > radius) return false;
//
// 	auto c = glm::vec3();
//
// 	// Edge 0
...
// 	glm::vec3 vp0 = hit_point - v0.position;
// 	c = cross(edge0, vp0);
// 	if (dot(face_normal, c) < 0) return false; // P is on the right side
//
// 	// Edge 1
// 	glm::vec3 edge1 = v2.position - v1.position;
// 	glm::vec3 vp1 = hit_point - v1.position;
