...
int Mesh3D::drawSelf()
{
	for (int i = 0; i < mesh_->vertexArrays.size(); ++i)
	{
		ShaderProgram* p = nullptr;
		if (i < this->materials_.size()) //check first materials of this 3D object
		{
			p = this->materials_[i];
		}
		else if (i < mesh_->materials.size()) //check material of mesh
		{
			p = mesh_->materials[i];
		}
