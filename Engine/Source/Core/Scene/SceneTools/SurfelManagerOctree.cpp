...
int SurfelManagerOctree::get_octree_level_for_surfel(const surfel* surfel)
{
	...
	return level_bounds - level_surfel ;
...
{
	auto p = surfel->mean;
	auto level = get_octree_level_for_surfel(surfel);

	glm::vec3 octree_center = glm::vec3(0, 0, 0);
	
	for (int i = 0; i < level; i++)
	{
		auto  o = octree_[i];
		auto p_centered = p - octree_center;
		auto affected_index = get_pos_of_next_surfel_index_(p_centered);
		if ((o.surfels_at_layer_amount & (1 << affected_index)) != 0)
		{
			//1 -> already set
...
unsigned int SurfelManagerOctree::get_surfel_buckets_from_ws_pos(glm::vec3 ws_pos, glm::vec3 ws_normal, unsigned int buckets[8])
{
...
glm::vec3 SurfelManagerOctree::get_surfel_bucket_center(glm::vec3 ws_pos) const
{
...
unsigned int SurfelManagerOctree::get_surfel_bucket_from_ws_pos(glm::vec3 ws_pos) const
{
	ws_pos /= SURFELS_BUCKET_SIZE;
